...
const SectionInfo = ({ elemInfo }) => {
	...
	return (
		...
		>
			...
			<p>{elemInfo.text}</p>
			{isAccent && elemInfo.accentHint ? (
				<span className={styles.accentHint}>{elemInfo.accentHint}</span>
			) : null}
			{elemInfo.desc ? <span className={styles.descInline}>{elemInfo.desc}</span> : null}
			{hovered && elemInfo.desc ? (
