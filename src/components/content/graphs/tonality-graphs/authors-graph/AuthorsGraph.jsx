...
const AuthorsGraph = ({ cashingData, isViewSource, onVisibleChange, rootName }) => {
	...
	const seriesData = useMemo(() => {
		if (!rootName) return cashingTransformAuthorsData;
		return cashingTransformAuthorsData.map(point => {
			if (!point) return point;
			if (point.id === 'root' || point.id === 'Root' || point.name === 'Root') {
				return { ...point, name: rootName };
			}
			return point;
		});
	}, [cashingTransformAuthorsData, rootName]);
	...
	const options = useMemo(
		...
		[seriesData, side, busy, reportVisible, rootLabel],
	);
